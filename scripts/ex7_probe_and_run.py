"""ex7 probe-and-run — the didactic tier-2 entrypoint.

This script is what `make ex7-real` does:

  1. Probe http://localhost:5005/version
  2. If Rasa answers → run the ex7 scenario against it
...
Why probe-then-run instead of auto-spawn?
  ...
  ex7 is where students first meet a multi-process agent system. The
  ...
  the whole point of ex7.

  If the student didn't start Rasa yet, we could auto-spawn it (`make
  ex7-auto` does exactly that). But auto-spawn hides the lesson. So
  `ex7-real` refuses to guess — it tells you "Rasa isn't up, here's how
...
def print_bootstrap_message(rasa_detail: str, actions_detail: str) -> None:
    ...
    print()
    print(_C.y("━" * 72))
    print(_C.b("  ex7 — Rasa isn't running yet"))
    print(_C.y("━" * 72))
    print()
    ...
    print("    " + _C.cyan("Terminal 3") + _C.d(" (this one):        ") + _C.b("make ex7-real"))
    print()
    ...
    print("    • Come back here and re-run " + _C.cyan("make ex7-real") + ".")
    print()
    ...
    print()
    print(_C.b("  In a hurry? ") + "There's an auto-spawn shortcut: " + _C.cyan("make ex7-auto"))
    ...
    print(_C.d("    teaches. Use ex7-real while you're learning.)"))
    print()
    print(_C.b("  Don't have a Rasa Pro license yet?"))
    print("    You can progress on ex7 using the mock server instead: " + _C.cyan("make ex7"))
    print(
...
def _print_notimpl_bootstrap(tail: str) -> None:
    ...
    print()
    print(_C.y("━" * 72))
    print(_C.b("  ex7 — Rasa is up, but YOUR code isn't complete yet"))
    print(_C.y("━" * 72))
    print()
    ...
    print()
    print("  When both are done, re-run " + _C.cyan("make ex7-real") + ".")
    print()
...
def main() -> int:
    ...
    if rasa_up and actions_up:
        ...
        print()
        print(_C.b("▶ Running Ex7 scenario..."))
        print()
        ...
        proc = subprocess.run(
            [sys.executable, "-m", "starter.handoff_bridge.run", "--real"],
            cwd=REPO,
            env={**os.environ},
            capture_output=True,
            text=True,
